...
FileName const hardcoded_localedir()
{
	...
	return FileName("/usr/local/share/locale");
}
...
FileName const hardcoded_system_support_dir()
{
	// FIXME UNICODE
	// The build system needs to make sure that this is in utf8 encoding.
	return FileName("/usr/local/share/lyx");
}


string const & with_version_suffix()
{
	static string const program_suffix("");
	static string const
		with_version_suffix(" --with-version-suffix=");
...
FileName const & Package::top_srcdir()
{
	// FIXME UNICODE
	// The build system needs to make sure that this is in utf8 encoding.
	static FileName const dir("/home/jspitzm/lyx/lyx-export");
	return dir;
}
